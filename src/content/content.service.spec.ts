...
describe('ContentService', () => {
  ...
  describe('findById', () => {
    it('should return cached content when available', async () => {
      ...
      expect(cacheManager.get).toHaveBeenCalledWith('content:1');
  ...
  describe('updateStatus', () => {
    it('should update status and invalidate cache', async () => {
      ...
      const result = await service.updateStatus(1, ContentStatus.PUBLISHED);

      expect(contentRepository.findOne).toHaveBeenCalledWith({
        where: { id: 1 },
      });
      expect(mockContent.status).toBe(ContentStatus.PUBLISHED);
      expect(cacheManager.del).toHaveBeenCalledWith('content:1');
    });
    ...
    it('should throw when content not found', async () => {
      ...
      await expect(
        service.updateStatus(999, ContentStatus.PUBLISHED),
